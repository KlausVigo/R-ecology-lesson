...
> it worked.
>
> > ```{r, answer=TRUE, echo=TRUE, eval=TRUE, purl=FALSE}
> >
> > # Let's try this on the `pop` column of the `gapminder` dataset.
> > # Make a new column in the `gapminder` data frame that
> > # contains population in units of millions of people.
...
>
> ```{r, answer=FALSE, echo=TRUE, eval=TRUE, purl=FALSE}
>
...
>
> ```{r, answer=FALSE, eval=TRUE, echo=TRUE, purl=FALSE}
>
> # Given the following matrix:
>
> m <- matrix(1:12, nrow=3, ncol=4)
...
> n=10,000?
>
> ```{r, answer=TRUE, eval=TRUE, echo=TRUE, purl=FALSE}
> 
> # We're interested in looking at the sum of the
> # following sequence of fractions:
> 
